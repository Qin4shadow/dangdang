...
<html>
  ...
  <body style="background-color: #f0f9fd;">
  	...
  	<div style="margin: 0 auto;text-align:center;">
  		<form action="${path }/book/checkBy" method="post">
  			<input onclick="location.href='${path}/book/insert2'" class="button btn-p" value="添加图书&raquo;" />&emsp;&emsp;
  	...
  	<table bordercolor='#898E90' align='center' border='3px' cellpadding='5px' cellspacing="0px" >
		...
		<tr align='center'>
			...
			<td>${li.ddPrice }</td>
			<td>${li.company }</td>
			<td><fmt:formatDate value="${li.publishTime }" pattern="yyyy-MM-dd"/></td>
			<td>
				<img width="32px" height="20px" src="${path }/back/photo/${li.src}" >
			</td>
			<td>${li.saleCount }</td>
			<td>${li.stock }</td>
			<td>
				<input  class="button"  onclick="location.href='${path}/book/checkOne?id=${li.id }'" value="修改 " />
				<input class="button-del" onclick="location.href='${path}/book/delete?id=${li.id }'" value="删除 " />
			</td>
		</tr>
		</c:forEach>
